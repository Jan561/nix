...
pub(super) mod private {
    ...
    pub unsafe trait SockaddrFromRawPriv {
        ...
        ///
        /// **Note**: This function must not fail if a length of zero is passed.
        ///
        /// # Safety
        ///
        /// One of the following must be true:
        ///
        /// - `addr` must be dereferencable to `Self::Storage`. In this case `len` must indicate
        /// the actual length of the address.
        /// - `addr` has been initialized with `Self::init_storage`. In this case `len` must be
        /// equal to the size of `Self::Storage`.
        unsafe fn from_raw(
...
/// Anything that represents a socket address.
///
/// Most implementors also implement `AsRef<libc::XXX>` to access their
/// inner type read-only.
///
/// **Note**: This trait is [sealed] and cannot be implemented outside of nix.
///
/// [sealed]: https://rust-lang.github.io/api-guidelines/future-proofing.html#sealed-traits-protect-against-downstream-implementations-c-sealed

// SAFETY: `&self` must have a minimum length of 2. This ensures that the family is always initialized
// and on some platforms also the length field.
...
pub unsafe trait SockaddrLike: private::SockaddrLikePriv {
    ...
    fn family(&self) -> AddressFamily {
        // SAFETY: `sa_family` is represented within the first two bytes of each address,
        // which is initialized as guaranteed by the safety invariant of this trait.
        unsafe {
            AddressFamily(*addr_of!((*(self as *const _ as *const libc::sockaddr)).sa_family) as _)
        }
    }
    ...
    cfg_if! {
        if #[cfg(not(any(
            ...
            fn len(&self) -> usize {
                // SAFETY: `sa_len` is the first byte, which is initialized as guaranteed
                // by the safety invariant of this trait.
                unsafe {
                    *addr_of!((*(self as *const _ as *const libc::sockaddr)).sa_len) as _
                }
            }
        } else {
            /// Return the length of valid data in the sockaddr structure.
...
///
/// Will be replaced by a type alias for `!` if that type ever [stabilizes].
///
...
unsafe impl private::SockaddrFromRawPriv for () {
    type Storage = ();
...
impl SockaddrFromRaw for () {}

// SAFETY: NoAddress can't be instantiated.
...
impl std::ops::Deref for UnixAddress {
    ...
    fn deref(&self) -> &Self::Target {
        ...
        unsafe {
            &*UnixAddr::from_raw_parts(&self.sun, self.len())
        }
    }
}
...
impl std::ops::DerefMut for UnixAddress {
    fn deref_mut(&mut self) -> &mut Self::Target {
        ...
        // SAFETY: The length cannot exceed the size of the struct.
        unsafe {
            &mut *UnixAddr::from_raw_parts_mut(&mut self.sun, len)
        }
    }
}
...
unsafe impl private::SockaddrFromRawPriv for Option<UnixAddress> {
    ...
    unsafe fn from_raw(
        ...
        cfg_if! {
            if #[cfg(debug_assertions)] {
                {
                    let addr = unsafe { &*UnixAddr::from_raw_parts(&*addr, len as _) };
...
impl SockaddrFromRaw for Option<UnixAddress> {}

unsafe impl private::SockaddrFromRawPriv for MaybeUninit<UnixAddress> {
    type Storage = libc::sockaddr_un;
...
impl SockaddrFromRaw for MaybeUninit<UnixAddress> {}
...
#[cfg(feature = "net")]
unsafe impl private::SockaddrFromRawPriv for Option<Ipv4Address> {
...
#[cfg(feature = "net")]
impl SockaddrFromRaw for Option<Ipv4Address> {}

#[cfg(feature = "net")]
unsafe impl private::SockaddrFromRawPriv for MaybeUninit<Ipv4Address> {
    type Storage = libc::sockaddr_in;
...
#[cfg(feature = "net")]
impl SockaddrFromRaw for MaybeUninit<Ipv4Address> {}
...
#[cfg(feature = "net")]
unsafe impl private::SockaddrFromRawPriv for Option<Ipv6Address> {
...
#[cfg(feature = "net")]
impl SockaddrFromRaw for Option<Ipv6Address> {}

#[cfg(feature = "net")]
unsafe impl private::SockaddrFromRawPriv for MaybeUninit<Ipv6Address> {
    type Storage = libc::sockaddr_in6;
...
#[cfg(feature = "net")]
impl SockaddrFromRaw for MaybeUninit<Ipv6Address> {}
...
impl Addr {
    ...
    pub const fn family(&self) -> AddressFamily {
        unsafe {
            AddressFamily(*addr_of!((*self.as_ptr()).ss_family) as _)
        }
    }
    ...
    pub fn to_link(&self) -> Option<&LinkAddr> {
        ...
        unsafe {
            Some(&*LinkAddr::from_raw_parts(self.as_ptr().cast(), self.len() as _))
        }
    }
    ...
    sockaddr_storage_conv!(to_sys_control, SysControlAddress, sockaddr_ctl, SYSTEM, "Converts to [`SysControlAddress`], if the address family matches.");
    ...
    pub fn to_unix(&self) -> Option<&UnixAddr> {
        ...
        unsafe {
            Some(&*UnixAddr::from_raw_parts(self.as_ptr().cast(), self.len() as _))
        }
    }
...
impl std::ops::Deref for Address {
    ...
    fn deref(&self) -> &Self::Target {
        unsafe {
            &*Addr::from_raw_parts(&self.storage, self.len())
        }
    }
}
...
impl std::ops::DerefMut for Address {
    fn deref_mut(&mut self) -> &mut Self::Target {
        ...
        unsafe {
            &mut *Addr::from_raw_parts_mut(&mut self.storage, len)
        }
    }
}
...
unsafe impl private::SockaddrFromRawPriv for Address {
...
impl SockaddrFromRaw for Address {}
...
pub mod netlink {
    ...
    unsafe impl private::SockaddrFromRawPriv for Option<NetlinkAddress> {
    ...
    impl SockaddrFromRaw for Option<NetlinkAddress> {}

    unsafe impl private::SockaddrFromRawPriv for MaybeUninit<NetlinkAddress> {
        type Storage = libc::sockaddr_nl;
    ...
    impl SockaddrFromRaw for MaybeUninit<NetlinkAddress> {}
...
pub mod alg {
    ...
    unsafe impl private::SockaddrFromRawPriv for Option<AlgAddress> {
    ...
    impl SockaddrFromRaw for Option<AlgAddress> {}

    unsafe impl private::SockaddrFromRawPriv for MaybeUninit<AlgAddress> {
        type Storage = libc::sockaddr_alg;
    ...
    impl SockaddrFromRaw for MaybeUninit<AlgAddress> {}
...
pub mod sys_control {
    ...
    unsafe impl private::SockaddrFromRawPriv for Option<SysControlAddress> {
    ...
    impl SockaddrFromRaw for Option<SysControlAddress> {}

    unsafe impl private::SockaddrFromRawPriv for MaybeUninit<SysControlAddress> {
        type Storage = libc::sockaddr_ctl;
    ...
    impl SockaddrFromRaw for MaybeUninit<SysControlAddress> {}

    impl private::SockaddrLikePriv for SysControlAddress {}

    unsafe impl SockaddrLike for SysControlAddress {
        sockaddr_len_static!();
    }
...
mod datalink {
    ...
    unsafe impl private::SockaddrFromRawPriv for Option<LinkAddress> {
    ...
    impl SockaddrFromRaw for Option<LinkAddress> {}

    unsafe impl private::SockaddrFromRawPriv for MaybeUninit<LinkAddress> {
        type Storage = libc::sockaddr_ll;
    ...
    impl SockaddrFromRaw for MaybeUninit<LinkAddress> {}
...
mod datalink {
    ...
    impl LinkAddr {
        ...
        ///
        /// This value can both be smaller or larger than [`SockaddrLike::len`].
...
pub mod vsock {
    ...
    unsafe impl private::SockaddrFromRawPriv for Option<VsockAddress> {
    ...
    impl SockaddrFromRaw for Option<VsockAddress> {}

    unsafe impl private::SockaddrFromRawPriv for MaybeUninit<VsockAddress> {
        type Storage = libc::sockaddr_vm;
    ...
    impl SockaddrFromRaw for MaybeUninit<VsockAddress> {}
...
impl<'a> RawAddr<'a> {
    ...
    raw_address_conv!(to_sys_control, SysControlAddress, sockaddr_ctl, SYSTEM, "Converts to [`SysControlAddress`], if the address family matches.");
