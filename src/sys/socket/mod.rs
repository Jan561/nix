...
pub use self::addr::{AddressFamily, InvalidAddressFamilyError, SockaddrFromRaw, SockaddrLike, SockaddrStorage, SockaddrStorageWrapper, NoAddress, UnixAddr};
...
/// assert_eq!(recv_res.address().map(|a| a.ip()), Some(addr.ip()));
///
...
impl CmsgVecWrite {
    ...
    ///
    /// This function does not allocate if the calculated `size` is smaller than the current [`Self::capacity`].
    ...
    /// # Panics
    ///
    /// Panics if the size needed to store the given control messages is larger than the current [`Self::capacity`].
    pub fn write_iter_in_place<'a, I>(&mut self, cmsg: I)
    where
        I: IntoIterator<Item = ControlMessage<'a>>,
    {
        self.write_iter(cmsg, self.inner.capacity()).expect("The control messages didn't fit into the current allocation");
    }
    ...
    C: CmsgBufRead + ?Sized,
{
    ...
    unsafe {
        ...
        addr_of_mut!((*msg_hdr_ptr).msg_namelen).write(S::size());
    ...
    C: CmsgBufRead + ?Sized + 'a,
{
    ...
    for (i, (iov, cmsg)) in items.take(buf.len()).enumerate() {
        let mmsg_hdr = libc::mmsghdr {
            msg_hdr: recvmsg_header(&mut addrs[i], iov, cmsg),
            msg_len: 0,
        };
    ...
    S: SockaddrLike,
{
        ...
        C: CmsgBufRead + ?Sized + 'a,
    {
        ...
        let size = recvmmsg_headers_into(mmsghdrs_uninit_slice, addresses_uninit_slice, items.by_ref());
...
where
    S: SockaddrLike,
{
    /// Creates a new container for a single `msg`-header.
    pub const fn new() -> Self {
        Self {
    ...
    where
        C: CmsgBufRead + ?Sized,
    {
        let msg_hdr = recvmsg_header(&mut self.address, iov, cmsg);
    ...
    S: SockaddrLike,
{
    ...
    pub fn address(&self) -> Option<S> {
        ...
        unsafe {
            S::from_raw(self.hdr.msg_name.cast_const().cast(), Some(self.hdr.msg_namelen))
        }
    }
...
mod test {
    ...
    fn test_recvmm_2() -> crate::Result<()> {
        use crate::sys::socket::{
            ...
            SockaddrIn, TimestampingFlag, CmsgEmpty, NoAddress,
        };
        use std::io::{IoSlice, IoSliceMut};
        ...
        let mut headers = super::RecvMmsgHeaders::<NoAddress>::with_capacity(recv_iovs.len());
