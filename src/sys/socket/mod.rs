...
//! of their length. The address length is bounded by `[2, SIZE]` with `SIZE` being the libc address type's size.
//! If an address is too large to fit into the struct, the instantiation will fail.
//!
//! - Addresses ending with `Addr` are unsized. They are generally *not* directly convertible to libc's address
//! types (they are convertible if and only if the address length matches the libc address type's size). The address
//! length is bounded by `[2, MAX]` with `MAX` being the maximum address length, i.e. the size of [`sockaddr_storage`].
//! Trying to instantiate or using an address with a length greater than `MAX` can result in a panic.
//!
...
impl<S> AddressBuffer<S>
where
    S: SockaddrFromRaw,
{
    /// Creates a new container for a single address.
    fn new() -> Self {
        let mut address = MaybeUninit::uninit();

        S::init_storage(&mut address);
    ...
    /// Returns a raw mutable pointer to the address buffer.
    fn as_mut_ptr(&mut self) -> *mut S::Storage {
    ...
    fn get(&self) -> S {
        unsafe {
            S::from_raw(self.address.as_ptr(), self.addr_len)
        }
    }
}

impl<S> Default for AddressBuffer<S>
where
    S: SockaddrFromRaw,
{
    fn default() -> Self {
...
impl<S> Clone for AddressBuffer<S>
where
    S: SockaddrFromRaw,
{
    fn clone(&self) -> Self {
...
impl<S> Copy for AddressBuffer<S>
where
    S: SockaddrFromRaw,
{}

impl<S> std::fmt::Debug for AddressBuffer<S>
where
    S: SockaddrFromRaw,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AddressBuffer")
...
unsafe impl<S> Send for AddressBuffer<S>
where
    S: SockaddrFromRaw,
{}

unsafe impl<S> Sync for AddressBuffer<S>
where
    S: SockaddrFromRaw,
{}
...
pub enum ControlMessageOwned {
    ...
    /// let address = getsockname::<Option<Ipv4Address>>(in_socket.as_raw_fd()).unwrap();
    ...
    /// let _ = recvmsg::<Option<Ipv4Address>>(
    ///     in_socket.as_raw_fd(),
    ///     &mut iov,
    ///     cmsg.handle(),
    ///     flags,
...
///
/// // Receive `msg` on `recv`.
/// let recv_res = recvmsg::<Option<Ipv4Address>>(
///     recv.as_raw_fd(),
...
pub fn recvmsg<S>(
    ...
    flags: MsgFlags,
) -> Result<RecvMsgResult<S>>
where
    S: SockaddrFromRaw,
{
    let mut addr_buf = AddressBuffer::new();

    let mut header = recvmsg_header::<S>(addr_buf.as_mut_ptr(), iov, &mut cmsg_buffer);
    ...
    addr_buf.addr_len = header.msg_namelen as _;
...
where
    S: SockaddrLike,
{
    let (addr_ptr, addr_len) = addr.filter(|a| a.len() != 0).map_or((ptr::null(), 0), |a| (a.as_sockaddr(), a.len()));
    ...
    S: SockaddrFromRaw,
{
    ...
    let addr_size = mem::size_of::<S::Storage>();
    ...
    S: SockaddrFromRaw,
{
    ...
    pub fn iter(&self) -> RecvMmsgIter<'_, S> {
        RecvMmsgIter {
        ...
        I: Iterator<Item = (&'a mut [IoSliceMut<'b>], CmsgBufHandle<'a>)> + ExactSizeIterator,
    {
        ...
        for _ in 0..len {
            ...
            self.addresses.push(AddressBuffer::new());
        }

        let mut addresses = self.addresses.iter_mut().map(AddressBuffer::as_mut_ptr);

        let mut total = 0;

        for (i, (iov, mut cmsg)) in items.by_ref().take(len).enumerate() {
            let mmsg_hdr = libc::mmsghdr {
                msg_hdr: recvmsg_header::<S>(addresses.next().unwrap(), iov, &mut cmsg),
                msg_len: 0,
            };
    ...
    unsafe fn update_lens(&mut self) {
            ...
            .take(self.recv)
        {
            ...
            addr.addr_len = mhdr.msg_hdr.msg_namelen as _;
        }
    }
}
...
impl<S> std::fmt::Debug for RecvMmsgHeaders<S>
where
    S: SockaddrFromRaw,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RecvMmsgHeaders")
            .field("mmsghdrs", &self.mmsghdrs)
...
pub struct RecvMmsgIter<'a, S>
where
    S: SockaddrFromRaw,
{
    addr: std::slice::Iter<'a, AddressBuffer<S>>,
    hdrs: std::slice::Iter<'a, libc::mmsghdr>,
}
...
impl<S> Iterator for RecvMmsgIter<'_, S>
where
    S: SockaddrFromRaw,
{
    type Item = RecvMsgResult<S>;
...
impl<S> Clone for RecvMmsgIter<'_, S>
where
    S: SockaddrFromRaw,
{
    fn clone(&self) -> Self {
...
impl<S> std::fmt::Debug for RecvMmsgIter<'_, S>
where
    S: SockaddrFromRaw,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RecvMmsgIter")
            .field("addr", &self.addr)
            .field("hdrs", &self.hdrs)
            .finish()
    }
}
...
unsafe impl<S> Send for RecvMmsgIter<'_, S>
where
    S: SockaddrFromRaw,
{}
...
unsafe impl<S> Sync for RecvMmsgIter<'_, S>
where
    S: SockaddrFromRaw,
{}
...
///
/// // We preallocate headers for 2 messages.
/// let mut recv_headers = RecvMmsgHeaders::<Option<Ipv4Address>>::new();
///
/// // Zip everything together.
/// let mut recv_items = [
...
pub fn recvmmsg<'a, 'b, S, J, I>(
    fd: RawFd,
    headers: &mut RecvMmsgHeaders<S>,
    items: J,
    flags: MsgFlags,
...
pub struct RecvMsgResult<S>
where
    S: SockaddrFromRaw,
{
    bytes: usize,
    hdr: libc::msghdr,
    addr_buf: AddressBuffer<S>,
}

impl<S> RecvMsgResult<S>
where
    S: SockaddrFromRaw,
{
...
impl<S> Clone for RecvMsgResult<S>
where
    S: SockaddrFromRaw,
{
    fn clone(&self) -> Self {
...
impl<S> Copy for RecvMsgResult<S>
where
    S: SockaddrFromRaw,
{}

impl<S> std::fmt::Debug for RecvMsgResult<S>
where
    S: SockaddrFromRaw,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RecvMsgResult")
            .field("bytes", &self.bytes)
            .field("hdr", &self.hdr)
            .field("addr_buf", &self.addr_buf)
            .finish()
    }
}

unsafe impl<S> Send for RecvMsgResult<S>
where
    S: SockaddrFromRaw,
{}

unsafe impl<S> Sync for RecvMsgResult<S>
where
    S: SockaddrFromRaw,
{}
...
pub fn recvfrom<T: SockaddrFromRaw>(
    ...
    unsafe {
        let mut addr = AddressBuffer::<T>::new();
        let mut len = mem::size_of::<T::Storage>() as socklen_t;
