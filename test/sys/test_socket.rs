...
pub fn test_timestamping() {
    use nix::sys::socket::{
        ...
        CmsgVecRead, ControlMessageOwned, MsgFlags, NoAddress, RecvMsgHeader,
...
mod recvfrom {
    ...
    pub fn udp_recvmmsg() {
        ...
        let mut headers =
            RecvMmsgHeaders::<SockaddrIn>::with_capacity(msgs.len());

        let res: Vec<RecvMsgResult<SockaddrIn>> = recvmmsg(
            rsock.as_raw_fd(),
            &mut headers,
    ...
    pub fn udp_recvmmsg_dontwait_short_read() {
        ...
        let mut headers =
            RecvMmsgHeaders::<SockaddrIn>::with_capacity(NUM_MESSAGES_SENT + 1);
...
pub fn test_recvif() {
    ...
    use nix::sys::socket::{bind, NoAddress, SockFlag, SockType, SockaddrIn};
    use nix::sys::socket::{getsockname, setsockopt, socket};
    use nix::sys::socket::{recvmsg, sendmsg, ControlMessageOwned, MsgFlags};
    use std::io::{IoSlice, IoSliceMut};
...
fn test_recvmmsg_timestampns() {
    ...
    let mut headers = RecvMmsgHeaders::<NoAddress>::with_capacity(1);

    let mut cmsgs = [CmsgVecRead::with_capacity(cmsgspace)];

    let r: Vec<RecvMsgResult<NoAddress>> = recvmmsg(
        in_socket.as_raw_fd(),
        &mut headers,
...
fn test_cmsg_vec_write_reserve_and_write_in_place() {
    ...
    vec.write_iter_in_place(cmsgs.iter().copied());
...
fn test_connection_mode_recv_address() {
    ...
    let mut header = RecvMsgHeader::<SockaddrIn>::new();
