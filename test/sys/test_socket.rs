...
pub fn test_timestamping() {
    ...
    let _ = recvmsg::<()>(rsock.as_raw_fd(), &mut iov2, cmsg.handle(), flags)
        .unwrap();
...
pub fn test_getsockname() {
    ...
    assert_eq!(
        Some(sockaddr),
        getsockname::<Option<UnixAddress>>(sock.as_raw_fd())
...
pub fn test_recvmsg_sockaddr_un() {
    ...
    let res = socket::recvmsg::<Option<UnixAddress>>(
        sock.as_raw_fd(),
        &mut iov,
        Default::default(),
        MsgFlags::empty(),
    ...
    assert_eq!(sockaddr, res.address().unwrap());
}
...
mod recvfrom {
    ...
    pub fn udp_recvmmsg() {
        ...
        let mut headers =
            RecvMmsgHeaders::<Option<Ipv4Address>>::with_capacity(msgs.len());
        ...
        for h in headers.iter() {
            assert_eq!(AddressFamily::INET, h.address().unwrap().family());
    ...
    pub fn udp_recvmmsg_dontwait_short_read() {
        ...
        let mut headers = RecvMmsgHeaders::<Option<Ipv4Address>>::with_capacity(
            NUM_MESSAGES_SENT + 1,
        );
...
pub fn test_sendmsg_ipv4sendsrcaddr() {
    ...
    let bound_sock_addr: Option<Ipv4Address> =
        getsockname::<Option<Ipv4Address>>(sock.as_raw_fd()).unwrap();
...
pub fn test_unnamed_unixdomain() {
    ...
    let addr_1: Option<UnixAddress> =
        getsockname::<Option<UnixAddress>>(fd_1.as_raw_fd())
            .expect("getsockname failed");
...
pub fn test_unnamed_unixdomain_autobind() {
    ...
    let addr: Option<UnixAddress> =
        getsockname::<Option<UnixAddress>>(fd.as_raw_fd())
            .expect("getsockname failed");
...
pub fn test_recv_ipv4pktinfo() {
    ...
    let sa: Option<Ipv4Address> =
        getsockname::<Option<Ipv4Address>>(receive.as_raw_fd())
            .expect("getsockname failed");
...
pub fn test_recvif_ipv6() {
    ...
    let sa: Option<Ipv6Address> =
        getsockname::<Option<Ipv6Address>>(receive.as_raw_fd())
            .expect("getsockname failed");
...
fn test_recvmsg_timestampns() {
    ...
    let address: Option<Ipv4Address> =
        getsockname::<Option<Ipv4Address>>(in_socket.as_raw_fd()).unwrap();
    // Get initial time
    ...
    let _ =
        recvmsg::<()>(in_socket.as_raw_fd(), &mut iov, cmsg.handle(), flags)
            .unwrap();
